...
int main(void)
{
	...
	// IRQ config
	USART_IRQConfig(USART1_IRQn, 5, ENABLE);
	...
	while (1)
	{
		...
		// Start TX and RX
		USART_ReceiveDataIT(pUARTHandle, rx_buffer, len);
		USART_SendDataIT(pUARTHandle, tx_buffer, len);

		// Wait until done
		while (!(tx_complete && rx_complete));

		// At this point, rx_buffer holds received data
		delay(1000);  // optional delay
	}
}

// User-defined application callback
